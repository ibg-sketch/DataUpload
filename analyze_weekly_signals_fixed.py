...
def analyze_period(data, period_name):
    ...
    win_rate = (len(wins) / len(tradeable) * 100) if len(tradeable) > 0 else 0
    ...
    print(f"   🎲 ВИНРЕЙТ (WIN/LOSS): {win_rate:.1f}%")
    ...
    for symbol in data['symbol'].value_counts().head(11).index:
    ...
    # По уровням confidence (исправленное)
    print(f"\n🎲 ПО УРОВНЯМ CONFIDENCE:")
    bins = [0, 0.6, 0.7, 0.8, 0.9, 1.01]  # Confidence уже в формате 0-1
    labels = ['<60%', '60-69%', '70-79%', '80-89%', '90-100%']
    
    data_copy = data.copy()
    data_copy['conf_bin'] = pd.cut(data_copy['confidence'], bins=bins, labels=labels, include_lowest=True)
    
    for conf_level in labels:
        conf_data = data_copy[data_copy['conf_bin'] == conf_level]
        if len(conf_data) > 0:
    ...
    if len(tradeable) >= 5:
        print(f"\n🏆 ТОП-5 ЛУЧШИХ СДЕЛОК:")
        top_5 = tradeable.nlargest(5, 'profit_pct')
        ...
        print(f"\n💔 ТОП-5 ХУДШИХ СДЕЛОК:")
        worst_5 = tradeable.nsmallest(5, 'profit_pct')
