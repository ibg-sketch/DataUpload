#!/usr/bin/env python3
"""
All-In Realistic Trading Simulation for November 17, 2025
Realistic strategy: Open position on first signal, after close - only trade NEW signals (ignore old ones)
Uses BingX fee structure: Entry 0.05% (taker), Exit TP 0.02% (maker), Exit SL/TTL 0.05% (taker)
...
def main():
    ...
    # Sort by timestamp
    today_signals = today_signals.sort_values('timestamp_sent').reset_index(drop=True)
    
    print(f"=== ALL-IN REALISTIC STRATEGY SIMULATION ===")
    ...
    print(f"Fee structure (BingX): Entry 0.05%, TP 0.02%, SL/TTL 0.05%")
    print(f"\nStrategy: After position close, only trade NEW signals (ignore old ones)")
    print()
    
    # Track state
    balance = INITIAL_BALANCE
    last_position_close_time = None  # When last position closed
    
    trades = []
    skipped_old_signals = 0
    
    for idx, signal in today_signals.iterrows():
        signal_timestamp = signal['timestamp_sent']
        
        # Skip signals that were generated BEFORE the last position closed
        # (only trade NEW signals after position close)
        if last_position_close_time is not None:
            if signal_timestamp <= last_position_close_time:
                skipped_old_signals += 1
                continue
        
        # Open new position on this signal
        result = simulate_position(signal, balance)
        
        # Calculate when this position closes
        ...
        # Print every trade
        print(f"Trade #{len(trades)} - Entry: {signal_timestamp.strftime('%H:%M:%S')}, Close: {position_close_time.strftime('%H:%M:%S')}")
    ...
    if len(trades_df) > 0:
        ...
        print(f"Signals skipped (old, before last position close): {skipped_old_signals}")
        print()
        ...
        trades_df.to_csv('allin_realistic_trades.csv', index=False)
        print(f"\nDetailed trades saved to: allin_realistic_trades.csv")
    else:
