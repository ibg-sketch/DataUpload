"""
Correct TTL analysis using same logic as find_optimal_config.py
"""
import pandas as pd
...
print(f"📊 Правильный анализ TTL exits")
...
print(f"   Сигналов пропущено: {signals_skipped}")
print()

# TTL analysis
ttl_positions = [p for p in closed_positions if p['exit_type'] == 'TTL']
ttl_wins = [p for p in ttl_positions if p['pnl'] > 0]
ttl_losses = [p for p in ttl_positions if p['pnl'] <= 0]

ttl_total = len(ttl_positions)
ttl_win_count = len(ttl_wins)
ttl_loss_count = len(ttl_losses)
ttl_wr = (ttl_win_count / ttl_total * 100) if ttl_total > 0 else 0

ttl_total_pnl = sum(p['pnl'] for p in ttl_positions)
ttl_wins_pnl = sum(p['pnl'] for p in ttl_wins)
ttl_losses_pnl = sum(p['pnl'] for p in ttl_losses)
...
if len(ttl_wins) > 0:
    ...
    for i, trade in enumerate(ttl_wins_sorted[:10], 1):
        print(f"{i:2d}. {trade['time'].strftime('%H:%M')} | {trade['symbol']:8s} | {trade['side']:4s} | PnL: ${trade['pnl']:+10.2f}")
